...
const Register = () => {
  ...
  // Store actions
  const { registerWithEmail, isLoading, error: authError } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        const userName = user.displayName || name || "Pengguna";
        ...
        <Card className="backdrop-blur-xl bg-white/10 border-white/20 text-white shadow-2xl">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full cursor-pointer bg-pink-600 hover:bg-pink-700 text-lg py-6 transition-all shadow-lg mt-6"
                disabled={isLoading}
              >
                {isLoading ? (
          ...
          <CardFooter className="flex flex-col gap-2">
            ...
            <Link to="/">
              <Button
                variant="link"
                className="text-violet-400 hover:text-violet-300 p-0 cursor-pointer"
              >
                Login di sini
              </Button>
            </Link>
